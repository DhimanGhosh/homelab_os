...
def start_plugin(plugin_id: str, env_file: str = ".env") -> None:
    settings, job_store, logger = _job_services(env_file)
    runtime = PluginRuntime(settings.runtime_installed_plugins_dir, settings.manifests_dir / "plugin_state.json")
...
@app.command("install-core-service")
def install_core_service(env_file: str = ".env") -> None:
    settings = load_settings(env_file)
    ensure_runtime_dirs(settings)
    CoreServiceManager(settings).install_service()
    typer.echo("Installed homelab-os-core.service")

@app.command("enable-core-service")
def enable_core_service(env_file: str = ".env") -> None:
    settings = load_settings(env_file)
    ensure_runtime_dirs(settings)
    CoreServiceManager(settings).enable_and_start()
    typer.echo("Enabled and started homelab-os-core.service")
