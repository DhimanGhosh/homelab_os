...
APP_VERSION = os.getenv("APP_VERSION", "7.1.3")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _text(self, text, code=200, ctype="text/plain; charset=utf-8", include_body=True):
    ...
    def _send_stream_headers(self, target: Path, start: int | None = None, end: int | None = None):
    ...
    def _serve_static(self, rel_path: str, head_only: bool):
        rel = rel_path.lstrip("/")
    ...
    def _handle_request(self, head_only: bool):
        ...
        if path == "/api/library":
            ...
            return self._json({
                "tracks": tracks,
                "playlists": playlists,
                "artist_playlists": auto_artist_playlists(tracks),
                "folders": folders_tree(tracks),
                "name": APP_NAME,
                "version": APP_VERSION
        ...
        if path.startswith('/api/stream/'):
            ...
            offset, length = self._send_stream_headers(target, start, end)
            if head_only:
                return
            try:
