...
function relToDownloadUrl(file, preferredName) {
  const base = file.download_url || ('/downloaded/' + file.relative_path.split('/').map(encodeURIComponent).join('/'));
  return preferredName ? base + '?filename=' + encodeURIComponent(preferredName) : base;
}

function relToOpenUrl(file) {
  return file.browser_open_url || ('/open/' + file.relative_path.split('/').map(encodeURIComponent).join('/'));
}
...
function getDraft(rel, file) {
  if (!state.fileDrafts[rel]) {
    state.fileDrafts[rel] = { newName: file.name, destination: state.defaultExternalSaveDir, operation: 'copy' };
  }
  return state.fileDrafts[rel];
}

function syncDraftInputs(root) {
  root.querySelectorAll('[data-new-name],[data-dest],[data-op]').forEach(el => {
...
function updateDownloadLink(rel) {
  const card = [...document.querySelectorAll('.file')].find(el => el.dataset.relativePath === rel);
  if (!card) return;
...
// ── Render jobs ────────────────────────────────────────────────────────────────
function renderJobs() {
  jobsEl.innerHTML = state.jobs.length ? '' : '<div class="hint">No jobs yet.</div>';
  state.jobs.forEach(job => {
    const el   = document.createElement('div');
    el.className = 'job';
    const log  = (job.log || []).slice(-6).join('\n');
    const kind = job.kind === 'convert'        ? 'Conversion'
               : job.kind === 'save-as'        ? 'Save elsewhere'
               : job.kind === 'upload-convert' ? 'Upload + convert'
               : 'Download';
    ...
    el.innerHTML = `
      <div class="section-title">
        <strong>${kind} · ${escapeHtml(job.status)}</strong>
        <span class="muted">${Math.round(job.progress || 0)}%</span>
      </div>
      <div class="progress"><div style="width:${job.progress || 0}%"></div></div>
      <div class="hint" style="margin-top:8px">${escapeHtml(job.message || '')}</div>
      ${resultBtn ? `<div class="action-row">${resultBtn}</div>` : ''}
      ${job.output_path ? `<div class="hint mono" style="margin-top:8px">${escapeHtml(job.output_path)}</div>` : ''}
      ${log ? `<div class="log">${escapeHtml(log)}</div>` : ''}
    `;
    jobsEl.appendChild(el);
  });
  document.querySelectorAll('[data-focus-file]').forEach(btn =>
    btn.addEventListener('click', () => focusSavedFile(btn.dataset.focusFile))
  );
}
...
function renderFiles() {
  ...
  state.files.forEach(file => {
    ...
    const externalDef  = escapeHtml(state.defaultExternalSaveDir || '/mnt/nas/media/music');
    const draft        = getDraft(rel, file);
    const el           = document.createElement('div');
    el.className       = 'file';
    ...
    el.innerHTML = `
      <div class="section-title">
        <strong>${name}</strong><span class="pill">${escapeHtml(file.kind)}</span>
      </div>
      <div class="hint mono">${escapeHtml(file.full_path)}</div>
      <div class="hint">${fmtBytes(file.size_bytes)}</div>
      <div class="file-actions">
        <a class="btn small" href="${relToOpenUrl(file)}" target="_blank">Open in browser</a>
        <a class="btn small primary" data-device-download href="${relToDownloadUrl(file, draft.newName)}" download="${escapeHtml(draft.newName)}">Download to this device</a>
        ${file.kind !== 'audio' ? `<button class="btn small ghost" data-convert="${relEsc}">Convert to MP3</button>` : ''}
      </div>
      <div class="save-elsewhere">
        <div class="save-note">Rename this file, choose a destination, then copy or move it into another Raspberry Pi/NAS path like <span class="mono">${externalDef}</span>.</div>
        <div class="inline-grid">
          <input list="destinations" data-new-name="${relEsc}" value="${escapeHtml(draft.newName)}" placeholder="Rename file" />
          <input list="destinations" data-dest="${relEsc}"     value="${escapeHtml(draft.destination)}" placeholder="Destination path e.g. /mnt/nas/media/music" />
          <select data-op="${relEsc}">
            <option value="copy" ${draft.operation === 'copy' ? 'selected' : ''}>Copy there</option>
            <option value="move" ${draft.operation === 'move' ? 'selected' : ''}>Move there</option>
          </select>
          <button class="btn small ghost" data-save-as="${relEsc}">Save elsewhere</button>
        </div>
      </div>
    `;
  ...
  document.querySelectorAll('[data-save-as]').forEach(btn =>
    btn.addEventListener('click', async () => {
      ...
      try {
        await api('/api/save-as', { method: 'POST', body: JSON.stringify({ relative_path: rel, new_name, destination_path, operation }) });
        await refresh(true);
        alert(`${operation === 'move' ? 'Move' : 'Copy'} queued for ${destination_path}`);
      } catch (e) {
...
function focusSavedFile(rel) {
  const target = [...document.querySelectorAll('.file')].find(el => el.dataset.relativePath === rel);
  if (!target) return;
...
function updateSummary(data) {
  state.defaultExternalSaveDir = data.default_external_save_dir || '/mnt/nas/media/music';
  ...
  $('#hostRoot').textContent    = data.server_save_root_host || data.server_save_root;
  $('#externalRoot').textContent = state.defaultExternalSaveDir;
  $('#allowedRoots').textContent = 'Allowed Pi/NAS roots: ' + (state.allowedSaveRoots.join(', ') || '/mnt/nas, /data');
  $('#deviceHint').textContent  = data.device_hint;
  $('#savedCount').textContent  = data.saved_files.length;
  $('#audioCount').textContent  = data.saved_files.filter(f => f.kind === 'audio').length;
  $('#dlCount').textContent     = data.jobs.filter(j => j.status === 'completed').length;

  const convertOptions = data.saved_files.filter(f => f.kind !== 'audio');
  $('#convertSelect').innerHTML = convertOptions
    .map(f => `<option value="${escapeHtml(f.relative_path)}">${escapeHtml(f.name)}</option>`)
...
async function refresh(forceFiles = false) {
  ...
  updateSummary(data);
  renderJobs();
  if (forceFiles || !editingInFiles()) { renderFiles(); }
  setRefreshStamp();
}
...
fetch('/api/health').then(r => r.json()).then(data => {
  ...
  if (appVersion) appVersion.textContent = `${version} • 8460 • social + file links + MP3 conversion`;
  if (pageVersion) pageVersion.textContent = version;
}).catch(() => {});

$('#clearJobsBtn').addEventListener('click', async () => {
  try {
    await api('/api/clear-clutter', { method: 'POST', body: '{}' });
    await refresh(true);
...
$('#startBtn').addEventListener('click', async () => {
  ...
  try {
    await api('/api/download', { method: 'POST', body: JSON.stringify({ url, mode: $('#modeSelect').value, audio_format: 'mp3' }) });
    $('#urlInput').value = '';
    await refresh(true);
  } catch (e) { alert(e.message || String(e)); }
  finally     { setBusy(false, 'Start download'); }
});

$('#convertBtn').addEventListener('click', async () => {
  const relative_path = $('#convertSelect').value;
  if (!relative_path) { alert('Select a saved video file first'); return; }
  try {
    await api('/api/convert', { method: 'POST', body: JSON.stringify({ relative_path, new_name: $('#convertNewName').value.trim() }) });
