...
def install(settings, extracted, meta):
    ...
    backend_app_dir = extracted / 'backend' / 'app'
    backend_requirements = backend_app_dir / 'requirements.txt'
    service_template = extracted / 'docker' / 'service' / 'control-center.service.template'
    cc_pkg = backend_app_dir / 'control_center_app' / 'web.py'
    if not cc_pkg.exists():
        raise RuntimeError(f'Missing Control Center app package: {cc_pkg}')
    if not backend_requirements.exists():
        raise RuntimeError(f'Missing Control Center requirements: {backend_requirements}')
    ...
    app_runtime_dir = current_dir / 'app'
    ...
    _run(['sudo', 'mkdir', '-p', app_runtime_dir])
    _run(['sudo', 'rsync', '-a', f'{backend_app_dir}/', f'{app_runtime_dir}/'])
    ...
    _run(['sudo', venv_python, '-m', 'pip', 'install', '-r', backend_requirements])
