...
EXPORT_COLUMNS = [
    'id', 'title', 'author', 'isbn', 'genre', 'subgenres', 'description', 'language', 'published_year', 'page_count',
    'publisher', 'info_link', 'cover_url', 'image_path', 'buy_link', 'mood', 'english_label', 'english_ease_score', 'india_set',
    'wow_score', 'emotional_score', 'sadness_score', 'realism_score', 'personalized_score', 'rating', 'status',
    'notes', 'source', 'current_page', 'bookmark_page', 'bookmark_note', 'created_at', 'updated_at'
]

SORT_FIELDS = {
    'title', 'author', 'genre', 'language', 'published_year', 'page_count', 'english_label', 'wow_score',
    'emotional_score', 'sadness_score', 'realism_score', 'personalized_score', 'status', 'bookmark_page', 'current_page',
    'rating', 'publisher', 'created_at', 'updated_at'
}
...
class UpdateBookRequest(BaseModel):
    ...
    english_ease_score: int | None = None
    india_set: str | None = None
    wow_score: int | None = None
    emotional_score: int | None = None
    sadness_score: int | None = None
...
class SettingsRequest(BaseModel):
    ...
    wow_weight: float | None = None
    emotion_weight: float | None = None
    sadness_weight: float | None = None
    realism_weight: float | None = None
    genre_bonus_weight: float | None = None
    genre_bonus_value: float | None = None
    genre_bonus_keywords: str | None = None
    score_formula_label: str | None = None
    recommendation_statuses: str | None = None
...
@app.get('/')
def root():
    return FileResponse('/opt/personal-library/static/index.html')
...
def should_use_enriched_as_primary(row: dict) -> bool:
    ...
    populated = sum(1 for f in rich_fields if str(row.get(f, '')).strip())
    return populated <= 2
...
def book_matches_query(book: dict, q: str) -> bool:
    ...
    if '=' in q:
        ...
        if field:
            raw = book.get(field, '')
            return value in str(raw).lower()
    hay = ' | '.join([
        str(book.get('title', '')), str(book.get('author', '')), str(book.get('genre', '')), str(book.get('subgenres', '')),
        str(book.get('language', '')), str(book.get('publisher', '')), str(book.get('notes', '')),
        str(book.get('mood', '')), str(book.get('english_label', '')), str(book.get('wow_score', '')),
        str(book.get('emotional_score', '')), str(book.get('sadness_score', '')), str(book.get('realism_score', '')),
        str(book.get('status', '')), str(book.get('bookmark_note', '')), str(book.get('buy_link', '')),
    ]).lower()
    return low in hay
...
def sort_books(rows: list[dict], sort_by: str, sort_dir: str):
    ...
    if sort_by != 'title':
        rows.sort(key=lambda x: keyfn(x), reverse=reverse)
    return rows


@app.get('/api/books')
def list_books(q: str = '', genre: str = '', status: str = '', sort_by: str = 'personalized_score', sort_dir: str = 'desc', bookmarked: bool = False):
    ensure_db()
    with connect() as conn:
    ...
    if bookmarked:
        rows = [r for r in rows if int(r.get('bookmark_page') or 0) > 0 or int(r.get('current_page') or 0) > 0 or str(r.get('bookmark_note') or '').strip()]
    if q:
        rows = [r for r in rows if book_matches_query(r, q)]
    rows = sort_books(rows, sort_by, sort_dir)
    return rows
...
def add_book(payload: AddBookRequest):
    ...
    book['notes'] = payload.notes.strip()
    book['status'] = 'Want to Read'
    book['current_page'] = 0
    book['bookmark_page'] = 0
    book['bookmark_note'] = ''
    with connect() as conn:
...
def update_book(book_id: int, payload: UpdateBookRequest):
    ...
    with connect() as conn:
        ...
        values = [merged[field] for field in fields] + [book_id]
...
def refresh_book(book_id: int):
    ...
    with connect() as conn:
        ...
        keep = {
            'status': current.get('status', 'Want to Read'),
            'notes': current.get('notes', ''),
            'current_page': current.get('current_page', 0),
            'bookmark_page': current.get('bookmark_page', 0),
            'bookmark_note': current.get('bookmark_note', ''),
        }
        merged = {**current, **enriched, **keep}
        merged['personalized_score'] = calculate_personalized_score(merged, settings)
        refresh_fields = [
            'author', 'isbn', 'genre', 'subgenres', 'description', 'language', 'published_year', 'page_count', 'publisher',
            'info_link', 'cover_url', 'buy_link', 'mood', 'english_label', 'english_ease_score', 'india_set', 'wow_score',
            'emotional_score', 'sadness_score', 'realism_score', 'personalized_score', 'rating', 'source', 'status', 'notes',
            'current_page', 'bookmark_page', 'bookmark_note'
        ]
        assignments = ', '.join([f'{field} = ?' for field in refresh_fields]) + ', updated_at = CURRENT_TIMESTAMP'
        values = [merged.get(field, '') for field in refresh_fields] + [book_id]
        conn.execute(f'UPDATE books SET {assignments} WHERE id = ?', values)
        updated = dict(conn.execute('SELECT * FROM books WHERE id = ?', (book_id,)).fetchone())
    return updated
...
def recommendation():
    ...
    allowed_statuses = {part.strip() for part in raw_statuses.split(',') if part.strip()}
    if not allowed_statuses:
        allowed_statuses = {'Want to Read', 'Paused'}

    def eligible_next(book):
        status = (book.get('status') or '').strip()
        return status in allowed_statuses and book.get('id') != current_id

    candidates = [b for b in rows if eligible_next(b)]
...
def sample_json():
    sample = [{
        'title': 'Sample Book', 'author': 'Sample Author', 'genre': 'Mystery', 'description': 'Short description',
        'language': 'ENGLISH', 'published_year': '2024', 'page_count': 240, 'buy_link': 'https://example.com',
        'wow_score': 4, 'emotional_score': 3, 'sadness_score': 2, 'realism_score': 4, 'status': 'Want to Read',
        'notes': 'Optional notes', 'bookmark_page': 0, 'bookmark_note': ''
    }]
...
def normalize_import_row(data: dict, settings: dict) -> dict:
    ...
    if should_use_enriched_as_primary(clean):
        ...
        for field in ['status', 'notes', 'current_page', 'bookmark_page', 'bookmark_note', 'buy_link', 'info_link', 'cover_url']:
            if str(clean.get(field, '')).strip():
                base[field] = clean[field]
        clean = {**clean, **base}
    else:
        for k, v in enriched.items():
            if not str(clean.get(k, '')).strip():
                clean[k] = v
    if enriched.get('title'):
        clean['title'] = enriched.get('title')
    if enriched.get('author'):
        clean['author'] = enriched.get('author')
    for field in ['wow_score', 'emotional_score', 'sadness_score', 'realism_score', 'english_ease_score', 'page_count', 'current_page', 'bookmark_page']:
        try:
            if clean.get(field, '') != '':
                clean[field] = int(float(clean[field]))
        except Exception:
            clean[field] = 0 if field in {'page_count', 'current_page', 'bookmark_page'} else 3
    if clean.get('status') not in STATUS_OPTIONS:
        clean['status'] = 'Want to Read'
        ...
        for idx, row in enumerate(rows, start=1):
            try:
                ...
                if existing:
                    ...
                    assignments = ', '.join([f'{field} = ?' for field in fields]) + ', updated_at = CURRENT_TIMESTAMP'
                    conn.execute(f'UPDATE books SET {assignments} WHERE id = ?', [clean[f] for f in fields] + [existing['id']])
                    summary['updated'] += 1
                else:
...
async def import_json(file: UploadFile = File(...)):
    ...
    summary = upsert_import_rows(rows)
    return summary
