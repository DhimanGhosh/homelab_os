...
from .config import GOOGLE_BOOKS_URL, OPEN_LIBRARY_SEARCH_URL, AMAZON_BASE, DEFAULT_SCORE_SETTINGS

BENGALI_RANGE = re.compile(r"[\u0980-\u09FF]")

INDIAN_PUBLISHERS = {
    'rupa', 'rupa publications', 'harpercollins publishers india', 'westland', 'penguin india',
    'penguin random house india', 'ananda publishers', 'anand publishers', 'dey\'s publishing',
    'sahitya akademi', 'rupa & co', 'rupa and co'
}
...
def choose_best(candidates: list[dict[str, Any]], title: str, author: str = '', isbn: str = '') -> dict[str, Any] | None:
    if not candidates:
        return None
    ranked = sorted(candidates, key=lambda item: score_candidate(item, title, author, isbn), reverse=True)
    return ranked[0]
...
def calculate_personalized_score(book: dict, settings: dict[str, Any] | None = None) -> float:
    ...
    raw_bonus = float(cfg.get('genre_bonus_value', 5.0) or 0) if any(k in (genre + ' ' + str(book.get('subgenres','')).lower()) for k in keywords) else 0.0
    score = (
...
def merge_sources(primary: dict[str, Any], fallback: dict[str, Any] | None) -> dict[str, Any]:
    if not fallback:
        return primary
    merged = dict(primary)
    for key in ['description', 'categories', 'language', 'publishedDate', 'pageCount', 'publisher', 'industryIdentifiers', 'infoLink', 'imageLinks', 'averageRating']:
...
def enrich_book(title: str, author: str = '', isbn: str = '', settings: dict[str, Any] | None = None) -> dict:
    ...
    book = {
        ...
        'cover_url': cover_url,
        'buy_link': build_amazon_link(final_title, final_author),
...
def derive_english_label(title, description, language=''):
    ...
    if avg_len < 4.8 and wps < 14:
        return 'Very Simple', 5
    if avg_len < 5.3 and wps < 18:
        return 'Simple', 4
    if avg_len < 6.0 and wps < 23:
        return 'Moderate', 3
    if avg_len < 6.8 and wps < 30:
        return 'Advanced', 2
    return 'Complex', 1
...
def looks_indian_author(author: str) -> bool:
    ...
    markers = ['bhagat','datta','bond','satyajit','tagore','mukherjee','chattopadhyay','chakraborty','banerjee','ghosh','ray','basu','anand','narayan','tharoor','amitav','rushdie','amish','tripathi']
...
def derive_india_set(title, author, description, categories, publisher, language=''):
    ...
    if looks_indian_author(author):
        return 'Yes'
    strong_category_markers = ['indic fiction', 'indian fiction', 'indian literature', 'bengali fiction', 'bangla']
    if any(marker in cat_text for marker in strong_category_markers):
        return 'Yes'
    partly_markers = ['partition', 'south asia', 'pakistan', 'lahore', 'bangladesh', 'dhaka']
    if any(marker in (cat_text + ' ' + (description or '').lower()) for marker in partly_markers):
        return 'Partly'
    return 'No'
...
def derive_scores(genre, description, title, categories, india_set, language=''):
    ...
    if any(w in text for w in ['thriller', 'mystery', 'murder', 'crime', 'killer', 'suspense', 'feluda', 'byomkesh']):
        wow = 5
    elif any(w in text for w in ['epic', 'mythology', 'science fiction', 'dystopia']):
        wow = 4
    if any(w in text for w in ['love', 'family', 'friendship', 'loss', 'grief', 'heart']):
        emotional = 5
    elif any(w in text for w in ['journey', 'identity', 'coming of age']):
        emotional = 4
    if any(w in text for w in ['tragic', 'death', 'war', 'partition', 'betrayal', 'grief', 'loss']):
        sadness = 5
    elif any(w in text for w in ['melancholy', 'lonely', 'broken']):
        sadness = 4
    elif any(w in text for w in ['romance', 'emotional']):
        sadness = 3
    if any(w in text for w in ['literary', 'family life', 'historical', 'contemporary', 'campus', 'realistic', 'bengali']):
        realism = 5
    elif any(w in text for w in ['mythology', 'science fiction', 'fantasy']):
        realism = 2
    elif any(w in text for w in ['thriller', 'mystery']):
        realism = 4
    if india_set == 'Yes':
