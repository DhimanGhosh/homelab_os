fetch("/api/health")
  .then((r) => r.json())
  .then((data) => {
    document.title = data.service || data.app_name || document.title;
    const appName = document.getElementById("appName");
    const appVersion = document.getElementById("appVersion");
    if (appName) appName.textContent = data.service || data.app_name || "";
    if (appVersion)
...
function hideBusy() {
  byId("busyOverlay").classList.remove("show");
}
function slugStatus(v) {
  return (
    "status-" +
    String(v || "")
      .toLowerCase()
...
function applyStatusClass(el) {
  if (!el) return;
  el.className = "status-select " + slugStatus(el.value);
}
function badge(v, p = "") {
  return `<span class="pill score-${Number(v) || 0}">${esc(p)}${esc(v)}</span>`;
}
...
function statusSelect(b) {
  return `<select class="status-select ${slugStatus(b.status)}" onchange="changeStatus(${b.id},this.value,this)">${statuses
    .slice(1)
...
function setSort(k) {
  if (k === "actions") return;
  state.sortDir =
    state.sortBy === k && state.sortDir === "desc" ? "asc" : "desc";
  state.sortBy = k;
  renderBooks();
}
...
function renderBooks() {
  ...
  byId("pageInfo").textContent =
    `Showing ${total ? start + 1 : 0}-${Math.min(start + slice.length, total)} of ${total}`;
  byId("thead").innerHTML = columns
    .map(
      ([k, l]) =>
  ...
  byId("tbody").innerHTML = slice
    ...
    .join("");
  byId("mobileList").innerHTML = slice
    .map(
      (b) =>
...
async function loadGenres() {
  ...
  byId("genre").innerHTML =
    "<option>All</option>" +
    genres.map((g) => `<option>${esc(g)}</option>`).join("");
  byId("status").innerHTML = statuses
...
function syncFilterText() {
  ...
  if (byId("status").value !== "All")
    parts.push(`status=${byId("status").value}`);
  if (state.bookmarkedOnly) parts.push("bookmarked=true");
  byId("activeFilterText").textContent = parts.length
    ? `Active filter: ${parts.join(" | ")}`
...
async function loadBooks(extra = {}) {
  ...
  const status =
    extra.status !== undefined ? extra.status : byId("status").value;
  if (q) p.set("q", q);
...
async function loadRecommendation() {
  ...
  const statuses = (data.allowed_statuses || []).join(", ") || "None";
  const note = `No recommendation yet. Current rule: ${statuses}.`;
  byId("currentReading").innerHTML = sideBookCard(
    data.current,
    "No book is currently marked as Reading.",
  );
  byId("recommendation").innerHTML =
    sideBookCard(data.next, note) +
    `<div class="small" style="margin-top:8px">${esc(data.rule_label || "Eligible statuses for automatic next recommendation")}: ${esc(statuses)}</div>`;
}
async function loadStats() {
  ...
  const items = [
    ...
    {
      label: "Not Bought",
      value: state.stats.statuses["Not Bought"] || 0,
      mode: "status",
      valueKey: "Not Bought",
      query: "status=Not Bought",
    },
    {
      label: "Want to Read",
      value: state.stats.statuses["Want to Read"] || 0,
      mode: "status",
      valueKey: "Want to Read",
      query: "status=Want to Read",
    },
    {
      label: "Reading",
      value: state.stats.statuses["Reading"] || 0,
      mode: "status",
      valueKey: "Reading",
    ...
    {
      label: "Paused",
      value: state.stats.statuses["Paused"] || 0,
      mode: "status",
      valueKey: "Paused",
      query: "status=Paused",
    },
    {
      label: "Read",
      value: state.stats.statuses["Read"] || 0,
      mode: "status",
      valueKey: "Read",
      query: "status=Read",
    },
  ...
  byId("stats").innerHTML = items
    ...
    .join("");
  byId("topGenres").innerHTML = state.stats.top_genres
    .map(
      (g) =>
...
function applyStatFilter(item) {
  ...
  if (item.mode === "all") {
    byId("q").value = "";
    loadBooks({ q: "", genre: "All", status: "All" });
    return;
  }
  if (item.mode === "status") {
    byId("q").value = item.query;
    byId("status").value = item.valueKey;
    loadBooks({ q: item.query, genre: "All", status: item.valueKey });
    return;
  }
  if (item.mode === "bookmarked") {
    state.bookmarkedOnly = true;
    byId("q").value = item.query;
    loadBooks({ q: "bookmarked=true", genre: "All", status: "All" });
    return;
  }
}
  ...
  try {
    const res = await api("/api/books", {
      ...
      body: JSON.stringify({
        ...
        isbn: byId("isbn").value.trim(),
        notes: byId("notes").value.trim(),
      }),
    });
    ["title", "author", "isbn", "notes"].forEach((id) => (byId(id).value = ""));
    if (res && res._duplicate_skipped)
      alert(res._message || "Duplicate skipped");
    await reloadAll();
  } catch (e) {
    alert(e.message);
  } finally {
...
async function dedupeBooks() {
  if (
    !confirm(
      "Remove newer duplicate rows and keep the oldest matching title + author?",
    )
  )
    return;
  showBusy(
    "Removing duplicates…",
    "Keeping the older matching title + author entry.",
  );
  try {
    const res = await api("/api/books/deduplicate", { method: "POST" });
    alert(
      `Removed ${res.removed_count} duplicate entr${res.removed_count === 1 ? "y" : "ies"}.`,
    );
    await reloadAll();
  } catch (e) {
    alert(e.message);
...
async function refreshBook(id) {
  showBusy(
    "Re-verifying book…",
    "Refreshing metadata, cover image, genre and description from online sources.",
  );
  try {
    await api(`/api/books/${id}/refresh`, { method: "POST" });
    await reloadAll();
  } catch (e) {
    alert(e.message);
  } finally {
...
async function openDetails(id) {
  ...
  byId("d_scores").innerHTML =
    `${badge(b.wow_score, "WOW ")}${badge(b.emotional_score, "Emotion ")}${badge(b.sadness_score, "Sadness ")}${badge(b.realism_score, "Realism ")}`;
  byId("d_status").innerHTML = statuses
    .slice(1)
    .map((s) => `<option ${s === b.status ? "selected" : ""}>${s}</option>`)
    .join("");
  applyStatusClass(byId("d_status"));
  byId("d_bookmark").textContent = b.bookmark_page
    ? `Page ${b.bookmark_page}${b.bookmark_note ? ` · ${b.bookmark_note}` : ""}`
    : b.current_page
      ? `Current page ${b.current_page}`
      : "—";
  byId("d_score").textContent = b.personalized_score || "";
  byId("d_genre").textContent =
    [b.genre, b.subgenres].filter(Boolean).join(" · ") || "—";
  byId("d_description").textContent = b.description || "—";
  byId("d_notes").textContent = b.notes || "—";
  byId("d_buy").style.display = b.buy_link ? "inline" : "none";
  byId("d_buy").href = b.buy_link || "#";
  byId("d_info").style.display = b.info_link ? "inline" : "none";
  byId("d_info").href = b.info_link || "#";
...
async function reverifyCurrentDetail() {
  if (!state.detailId) return;
  await refreshBook(state.detailId);
  await openDetails(state.detailId);
}
async function changeDetailStatus(status, el) {
  if (!state.detailId) return;
  await changeStatus(state.detailId, status, el);
  await openDetails(state.detailId);
}
async function openEdit(id) {
  state.editId = id;
  const b =
    state.books.find((x) => x.id === id) || (await api(`/api/books/${id}`));
  [
    ["e_title", "title"],
    ["e_author", "author"],
    ["e_genre", "genre"],
    ["e_subgenres", "subgenres"],
    ["e_language", "language"],
    ["e_year", "published_year"],
    ["e_pages", "page_count"],
    ["e_complexity", "english_ease_score"],
    ["e_wow", "wow_score"],
    ["e_emotion", "emotional_score"],
    ["e_sadness", "sadness_score"],
    ["e_realism", "realism_score"],
    ["e_current", "current_page"],
    ["e_bookmark", "bookmark_page"],
    ["e_buy", "buy_link"],
    ["e_info", "info_link"],
    ["e_cover", "cover_url"],
    ...
    ["e_notes", "notes"],
    ["e_bookmark_note", "bookmark_note"],
    ["e_isbn", "isbn"],
  ].forEach(([id, key]) => (byId(id).value = b[key] ?? ""));
  byId("e_status").innerHTML = statuses
    .slice(1)
    .map((s) => `<option ${s === b.status ? "selected" : ""}>${s}</option>`)
    .join("");
  const br = await api(`/api/books/${id}/score-breakdown`);
  byId("scoreBreakdown").innerHTML =
    `<div><b>${esc(br.formula)}</b></div>` +
    Object.entries(br.components)
      .map(([k, v]) => `<div>${esc(k.replaceAll("_", " "))}: ${esc(v)}</div>`)
      .join("");
  byId("editDialog").showModal();
}
async function saveEdit() {
  const payload = {
    title: byId("e_title").value,
    author: byId("e_author").value,
    isbn: byId("e_isbn").value,
    genre: byId("e_genre").value,
    subgenres: byId("e_subgenres").value,
    language: byId("e_language").value,
    published_year: byId("e_year").value,
    page_count: +(byId("e_pages").value || 0),
    english_ease_score: +(byId("e_complexity").value || 3),
    wow_score: +(byId("e_wow").value || 3),
    emotional_score: +(byId("e_emotion").value || 3),
    sadness_score: +(byId("e_sadness").value || 2),
    realism_score: +(byId("e_realism").value || 3),
    current_page: +(byId("e_current").value || 0),
    bookmark_page: +(byId("e_bookmark").value || 0),
    buy_link: byId("e_buy").value,
    info_link: byId("e_info").value,
    cover_url: byId("e_cover").value,
    publisher: byId("e_publisher").value,
    description: byId("e_description").value,
    notes: byId("e_notes").value,
    bookmark_note: byId("e_bookmark_note").value,
    status: byId("e_status").value,
  };
  await api(`/api/books/${state.editId}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
async function openSettings() {
  ...
  const enabled = new Set(
    String(state.settings.recommendation_statuses || "Want to Read, Paused")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean),
  );
  document.querySelectorAll("[data-rec-status]").forEach((el) => {
    el.checked = enabled.has(el.dataset.recStatus);
  });
  const panels = [
    ...document.querySelectorAll("#settingsDialog details.settings-accordion"),
  ];
  panels.forEach((el, i) => {
    el.open = i === 0;
  });
...
async function saveSettings() {
  ...
  [
    ...
    "genre_bonus_weight",
    "genre_bonus_value",
    "genre_bonus_keywords",
    "score_formula_label",
    "recommendation_explain_label",
  ].forEach((k) => {
    const el = byId("s_" + k);
    if (el) payload[k] = el.type === "number" ? Number(el.value) : el.value;
  });
  payload.recommendation_statuses = [
    ...document.querySelectorAll("[data-rec-status]:checked"),
  ]
    .map((el) => el.dataset.recStatus)
    .join(", ");
...
function triggerImport(type) {
  byId(type === "csv" ? "importCsv" : "importJson").click();
}
async function uploadImport(type, file) {
  showBusy(
    type === "csv" ? "Importing CSV…" : "Importing JSON…",
    "Matching titles, fetching metadata, checking duplicates and saving your library.",
  );
  ...
  try {
    ...
    alert(
      data.received !== undefined
        ? `Imported ${data.received} row(s). Inserted: ${data.inserted || 0}, Updated: ${data.updated || 0}, Skipped: ${data.skipped || 0}.`
        : "Import complete.",
    );
    await reloadAll();
  } catch (e) {
    ...
    hideBusy();
    byId(type === "csv" ? "importCsv" : "importJson").value = "";
  }
}
async function backupNow() {
  const r = await api("/api/backup", { method: "POST" });
  alert("Backup created: " + r.backup_path);
}
async function openBackups() {
  ...
  byId("backupList").innerHTML = (data.items || []).length
    ? data.items
      .map(
        (b) =>
          `<div class="card" style="margin-bottom:8px"><div><b>${esc(b.name)}</b></div><div class="small">${esc(b.modified_at)} · ${Math.round((b.size || 0) / 1024)} KB</div><div class="row" style="margin-top:8px"><button class="secondary" onclick="restoreBackup('${esc(b.name)}')">Restore</button><button class="secondary" onclick="deleteBackup('${esc(b.name)}')">Remove</button></div></div>`,
      )
      .join("")
...
async function restoreBackup(name) {
  if (!confirm(`Restore backup ${name}? Current DB will be backed up first.`))
    return;
  showBusy(
    "Restoring backup…",
    "Restoring the selected backup and reloading the app.",
  );
  try {
...
function setupSettingsAccordion() {
  document
    .querySelectorAll("#settingsDialog details.settings-accordion > summary")
    .forEach((summary) => {
      summary.onclick = (e) => {
        e.preventDefault();
        ...
        document
          .querySelectorAll("#settingsDialog details.settings-accordion")
          .forEach((el) => {
            el.open = false;
          });
...
byId("pageSize").addEventListener("change", () => {
  state.page = 1;
  renderBooks();
});
byId("importCsv").addEventListener(
  "change",
  (e) => e.target.files[0] && uploadImport("csv", e.target.files[0]),
);
byId("importJson").addEventListener(
  "change",
  (e) => e.target.files[0] && uploadImport("json", e.target.files[0]),
);
reloadAll();
