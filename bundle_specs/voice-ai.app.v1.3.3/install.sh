...
if [[ -x "$ROOT/runtime/init.sh" ]]; then
...
curl -kfsS --max-time 5 "$HEALTH_URL" >/dev/null 2>&1 || { echo "Local service health check failed: $HEALTH_URL" >&2; sudo docker compose logs --tail=200 || true; exit 1; }
