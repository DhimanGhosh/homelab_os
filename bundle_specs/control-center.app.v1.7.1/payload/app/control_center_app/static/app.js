...
function isUninstallJob(job) {
  return !!(
    job &&
    (job.action === "uninstall" ||
      /uninstall|stopping|remov/i.test(String(job.message || "")))
  );
}
function queueLabel(job) {
  if (!job) return "Working";
  if (isUninstallJob(job))
    return job.status === "queued" ? "Queued uninstall" : "Uninstalling";
  return job.status === "queued" ? "Queued" : "Installing";
}
...
function syncSelectedLogBox() {
  ...
  if (!box) return;
  selectedLogAutoScroll =
    box.scrollHeight - box.clientHeight - box.scrollTop < 24;
}
async function refreshSelectedLog() {
  if (!selectedLogName) return;
  const res = await fetch(
    "/api/logs/" + encodeURIComponent(selectedLogName) + "?ts=" + Date.now(),
    { cache: "no-store" },
  );
  if (!res.ok) return;
...
function renderJobs() {
  ...
  const jobs = (state.jobs || [])
    ...
    .sort(
      (a, b) =>
        ["running", "queued", "failed", "success"].indexOf(a.status) -
        ["running", "queued", "failed", "success"].indexOf(b.status) ||
        String(b.updated_at).localeCompare(String(a.updated_at)),
    );
  if (!jobs.length) {
    area.innerHTML = '<div class="muted">No install jobs yet.</div>';
    return;
  }
  area.innerHTML = jobs
    .map(
...
function appCard(app) {
  const bundles =
    (app.bundles || [])
      .map(
        (b) =>
          `<div class="bundle-row"><div><div>${esc(b.filename)}</div><div class="muted">v${esc(b.version)}</div></div><div class="bundle-actions"><button class="secondary" data-install-bundle="${esc(app.id)}" data-bundle-filename="${esc(b.filename)}">Install</button><button class="danger" data-delete-bundle="${esc(b.filename)}">Delete</button></div></div>`,
      )
      .join("") || '<div class="muted">No uploaded bundles.</div>';
  const job = app.job;
  const open = app.open_url
    ? `<a class="btn secondary" target="_blank" href="${esc(app.open_url)}">Open</a>`
    : "";
  let action = "";
  if (job) {
    action = `<button disabled>${esc(queueLabel(job))}</button>`;
  } else if (["install", "update", "reinstall"].includes(app.action)) {
    action = `<button data-install="${esc(app.id)}">${fmtAction(app.action)}</button>`;
  }
  const uninstall =
    app.id !== "control-center" && app.installed && !job
      ? `<button data-uninstall="${esc(app.id)}">Uninstall</button>`
      : "";
  const progress = job
    ? `<div class="progress compact"><div class="bar ${statusClass(job.status)}" style="width:${pct(job.progress)}%"></div></div><div class="muted small">${esc(job.message || "")}</div>`
    : "";
  const noteBadge = app.notification_count
    ? `<button class="app-notif-badge app-note-trigger" title="Show notifications" data-show-app-notifications="${esc(app.id)}">${app.notification_count}</button>`
    : "";
...
function renderApps() {
  document.getElementById("appsArea").innerHTML = (state.apps || [])
    .map(appCard)
    .join("");
  document.querySelectorAll("#appsArea details.bundles-block").forEach((el) => {
    el.open = openBundleId && el.dataset.appId === openBundleId;
  });
...
function renderNotifications() {
  ...
  area.innerHTML = notes.length
    ? notes
      .map(
        (n) =>
          `<div class="notice notice-banner"><div><strong>${esc(n.app_id || "system")}</strong></div><div>${esc(n.ts)} — ${esc(n.message)}</div></div>`,
      )
      .join("")
  ...
  if (notificationFilterAppId) {
    const app = (state.apps || []).find(
      (a) => a.id === notificationFilterAppId,
    );
    title.textContent =
      (app?.name || notificationFilterAppId) + " notifications";
...
function renderBackups() {
  ...
  if (!items.length) {
    area.innerHTML = '<div class="muted">No homelab snapshots yet.</div>';
    return;
  }
  area.innerHTML = items
    .map(
      (b) =>
        `<div class="backup-row"><div><div><strong>${esc(b.filename)}</strong></div><div class="muted small">${esc(b.created_at)} • ${esc(b.size_mb)} MB</div><div class="muted small">${esc(b.path || state.backup_root || "")}</div></div><div class="bundle-actions"><button class="secondary" data-restore-backup="${esc(b.filename)}">Rollback</button><button class="danger" data-delete-backup="${esc(b.filename)}">Delete</button></div></div>`,
    )
    .join("");
...
async function queueAction(url, options = {}, ui = {}) {
  ...
  if (uploadMsg && ui.clearUploadMsg !== false && !ui.useUploadMsg) {
    uploadMsg.textContent = "";
  }
  if (uploadMsg && ui.useUploadMsg) {
...
async function watchForOtaRestart(previousVersion) {
  ...
  otaWatchTimer = setInterval(async () => {
    ...
    try {
      ...
      if (
        sawFailure ||
        (previousVersion && newVersion && newVersion !== previousVersion)
      ) {
        clearInterval(otaWatchTimer);
        otaWatchTimer = null;
...
document.addEventListener("click", async (e) => {
  ...
  if (t.dataset.showAppNotifications) {
    e.preventDefault();
    toggleNotifications(true, t.dataset.showAppNotifications);
    return;
  }
  if (t.id === "updateAllBtn") {
    e.preventDefault();
    if (!confirm("Queue all available updates now?")) return;
    const prev = state.current_version;
    const out = await queueAction(
      "/api/update-all",
      { method: "POST" },
      { useQueueMsg: true },
    );
    if (
      out.res.ok &&
      /Control Center OTA started/.test(out.data.message || "")
    ) {
      watchForOtaRestart(prev);
    }
    return;
  }
  if (t.id === "installAllBtn") {
    e.preventDefault();
    if (!confirm("Queue install/update for all available bundles now?")) return;
    const prev = state.current_version;
    const out = await queueAction(
      "/api/install-all",
      { method: "POST" },
  ...
  if (t.dataset.showLog) {
    e.preventDefault();
    const txt = await fetch(
      "/api/logs/" +
      encodeURIComponent(t.dataset.showLog) +
      "?ts=" +
      Date.now(),
      { cache: "no-store" },
    ).then((r) => r.text());
    showLog(txt, t.dataset.showLog);
    return;
  }
  if (t.dataset.action === "rescan") {
    e.preventDefault();
    await queueAction(
  ...
  if (t.dataset.action === "clear-notifications") {
    e.preventDefault();
    await queueAction(
      "/api/notifications/clear",
      { method: "POST" },
  ...
  if (
    t.id === "clearCompletedBtn" ||
    t.dataset.action === "clear-completed-jobs"
  ) {
    e.preventDefault();
    await queueAction(
  ...
  if (t.id === "clearAllBtn") {
    e.preventDefault();
    if (!confirm("Clear all install queue jobs?")) return;
    await queueAction(
      "/api/jobs/clear-all",
      { method: "POST" },
      { silent: true },
  ...
  if (t.dataset.cancelJob) {
    e.preventDefault();
    await queueAction(
      "/api/jobs/" + encodeURIComponent(t.dataset.cancelJob) + "/cancel",
      { method: "POST" },
      { silent: true },
    );
    return;
  }
  if (t.dataset.dismissJob) {
    e.preventDefault();
    await queueAction(
      "/api/jobs/" + encodeURIComponent(t.dataset.dismissJob) + "/dismiss",
      { method: "POST" },
      { silent: true },
    );
    return;
  }
  if (t.dataset.action === "ota") {
    e.preventDefault();
    const prev = state.current_version;
    const out = await queueAction(
      "/api/ota/apply",
  ...
  if (t.dataset.install) {
    e.preventDefault();
    await queueAction(
      "/api/apps/" + encodeURIComponent(t.dataset.install) + "/install",
      { method: "POST" },
      { silent: true },
    );
    return;
  }
  if (t.dataset.installBundle) {
    e.preventDefault();
    const appId = t.dataset.installBundle;
    const filename = t.dataset.bundleFilename;
    await queueAction(
      "/api/apps/" +
  ...
  if (t.dataset.uninstall) {
    e.preventDefault();
    await queueAction(
      "/api/apps/" + encodeURIComponent(t.dataset.uninstall) + "/uninstall",
      { method: "POST" },
      { silent: true },
    );
    return;
  }
  if (t.dataset.deleteBundle) {
    e.preventDefault();
    if (!confirm("Delete bundle " + t.dataset.deleteBundle + "?")) return;
    const msg = document.getElementById("uploadMsg");
    if (msg) msg.textContent = "";
    const { res } = await apiJson(
      "/api/bundles/" + encodeURIComponent(t.dataset.deleteBundle),
      { method: "DELETE" },
    );
    if (res.ok) {
      await refreshState();
  ...
  if (t.dataset.restoreBackup) {
    e.preventDefault();
    if (
      !confirm("Rollback entire homelab from " + t.dataset.restoreBackup + "?")
    )
      return;
    await queueAction(
      "/api/backups/" +
      encodeURIComponent(t.dataset.restoreBackup) +
      "/restore",
      { method: "POST" },
      { silent: true },
    );
    watchForOtaRestart(state.current_version);
    return;
  }
  if (t.dataset.deleteBackup) {
    e.preventDefault();
    if (!confirm("Delete snapshot " + t.dataset.deleteBackup + "?")) return;
    await queueAction(
      "/api/backups/" + encodeURIComponent(t.dataset.deleteBackup),
      { method: "DELETE" },
      { silent: true },
    );
    return;
...
document
  .getElementById("logBox")
  ?.addEventListener("scroll", syncSelectedLogBox);

document.addEventListener("submit", async (e) => {
  if (e.target.id === "uploadForm") {
    e.preventDefault();
    const fd = new FormData(e.target);
    const { res, data } = await apiJson("/api/bundles/upload", {
      method: "POST",
      body: fd,
    });
...
document
  .getElementById("menuNotificationsBtn")
  ?.addEventListener("click", () => toggleNotifications(true));
document
  .getElementById("menuRescanBtn")
  ?.addEventListener("click", () =>
    queueAction("/api/marketplace/rescan", { method: "POST" }),
  );
...
document.getElementById("dropZone")?.addEventListener("drop", (e) => {
  ...
  e.currentTarget.classList.remove("dragover");
  e.currentTarget.classList.toggle("has-files", input.files.length > 0);
});
document.getElementById("bundleInput")?.addEventListener("change", (e) => {
  document
    .getElementById("dropZone")
    ?.classList.toggle("has-files", !!e.target.files.length);
});
...
async function copySelectedLog() {
  const text =
    document.getElementById("logBox")?.textContent || "No log available.";
  await copyText(text);
}
...
function setPullRefreshVisible(visible, text) {
  const el = ensurePullRefreshIndicator();
  el.textContent = text || "Release to refresh";
...
function handleTouchMove(e) {
  ...
  if (dy > 70) setPullRefreshVisible(true, "Release to refresh");
  else if (dy > 20) setPullRefreshVisible(true, "Pull to refresh");
  else setPullRefreshVisible(false);
}
...
document.getElementById("copyLogBtn")?.addEventListener("click", async () => {
  try {
    await copySelectedLog();
  } catch (_) { }
});
document.getElementById("appSearchInput")?.addEventListener("input", (e) => {
  appSearchTerm = (e.target.value || "").trim().toLowerCase();
  const clearBtn = document.getElementById("clearAppSearchBtn");
  if (clearBtn) clearBtn.style.display = appSearchTerm ? "inline-flex" : "none";
  renderApps();
});
...
document.getElementById("clearAppSearchBtn")?.addEventListener("click", () => {
  const input = document.getElementById("appSearchInput");
  if (!input) return;
  input.value = "";
  appSearchTerm = "";
  const btn = document.getElementById("clearAppSearchBtn");
