...
def serve_static_file(handler: BaseHTTPRequestHandler, relative_path: str):
    file_path = (STATIC_DIR / relative_path).resolve()
    if not file_path.exists() or not file_path.is_file() or (STATIC_DIR != file_path.parent and STATIC_DIR not in file_path.parents):
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path.startswith('/static/'):
            ...
            return serve_static_file(self, rel)
        if path == '/api/health':
