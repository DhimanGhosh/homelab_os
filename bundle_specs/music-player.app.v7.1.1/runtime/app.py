...
.np{min-width:0}.controls{display:flex;align-items:center;gap:10px;justify-content:center;flex-wrap:wrap}.bar{width:100%}button{background:#1b2331;border:1px solid var(--line);color:var(--text);padding:10px 12px;border-radius:12px;cursor:pointer}button:hover{filter:brightness(1.08)}.tiny{padding:7px 10px;font-size:12px}.sideSection{margin-top:18px}.miniList button{margin:4px 0;width:100%}.mobileOnly{display:none}
@media (max-width:900px){.app{grid-template-columns:1fr}.sidebar{display:none;position:fixed;inset:0 auto 96px 0;width:82vw;max-width:320px;z-index:20}.sidebar.open{display:block}.mobileOnly{display:inline-block}.player{grid-template-columns:1fr;height:auto;padding:10px 12px;gap:8px}}
</style></head><body>
<div class='app'><aside id='sidebar' class='sidebar'><div class='brand'>Music Player</div>
...
function renderSavedPlaylists(){const host=$('#savedPlaylists'); host.innerHTML=''; state.playlists.forEach(p=>{const b=document.createElement('button'); b.className='navbtn'; b.textContent=`${p.name} (${p.count})`; b.onclick=()=>renderList(p.tracks.map(id=>state.tracks.find(t=>t.id===id)).filter(Boolean),p.name); host.appendChild(b);});}
function card(title,sub,onclick){const d=document.createElement('div'); d.className='card'; d.innerHTML=`<div class='title'>${title}</div><div class='sub'>${sub}</div>`; d.onclick=onclick; return d;}
function row(track,index){const d=document.createElement('div'); d.className='row'; d.innerHTML=`<div class='meta'><div class='title'>${track.title}</div><div class='sub'>${track.artist} • ${track.folder||'Music'}</div></div><div class='pill'></div>`; d.onclick=()=>playFromList(state.filtered,index); return d;}
...
function setView(view){state.view=view; $$('.navbtn[data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view===view)); if(view==='all') renderList(applySearch(state.tracks)); else if(view==='artists') renderArtists(); else if(view==='folders') renderFolders(); else renderPlaylists(); $('#sidebar').classList.remove('open');}
...
class Handler(BaseHTTPRequestHandler):
    server_version = 'MusicPlayer/7.1.1'
